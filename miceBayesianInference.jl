...
# ╔═╡ 27312579-5d92-4872-83ef-8f022bbb612c
using Turing, Distributions, StatsPlots, MAT

# ╔═╡ 533579cb-674d-4deb-91c5-317ec51de5e1
const b = 0.7108

# ╔═╡ 3825f801-bff6-4849-835b-8a1f4593f847
const c0 = 5.0e4

# ╔═╡ 6da7c235-8ada-4ed0-932d-33357e78c543
const c✶ = 8000.0

# ╔═╡ e8baef3e-4167-42aa-afea-bfa820afcc38
function visible_radius(t; D, ρ, b, c✶, c0)
    a = (ρ * t - log((c✶ * (b^2 + 4D * t)^(3 / 2)) / (c0 * b^3)))
    r = a < 0 ? 0*a : √(a * (b^2 + 4D * t))
end

# ╔═╡ 208c6254-0f53-49fb-a4ab-2ad21b99dfae
@model function mouse(t, r✶)

    σ² ~ Exponential(0.1) 
    D ~ Uniform(0, 0.2)
    ρ ~ Uniform(0.05, 0.25)

    r✶_pred = visible_radius.(t; D, ρ, b, c✶, c0)

    for i = 1:length(r✶)
        r✶[i] ~ Normal(r✶_pred[i], sqrt(σ²))
    end

end

# ╔═╡ 452156c9-6e46-421e-ae63-6e4f145a54df
function main(id)

days = [7,14,19,21]

radii = [
    [0.471993983343538,0.843243482223551,1.579980345012241,1.953125000000000],
    [0.646599428017455,1.175125699725885,2.223476479805892,2.624465161287629],
    [0.983568618126107,2.016548359240673,2.894317742631673]
]

mice = [mouse(days, radii[1]), mouse(days, radii[2]), mouse(days[1:3], radii[3])]

chain = sample(mice[id], NUTS(), MCMCThreads(), 100000, 4)


end

# ╔═╡ 5b02a45f-3cba-4aa2-b4c1-7189c043a2a1


# ╔═╡ 09567e98-338f-4155-8349-3dc3d2cef2da
chain1 = main(1)

# ╔═╡ 84ac7a14-daac-4d32-8290-5850952f1062
C1 = Array(chain1)

# ╔═╡ 71040bc8-2608-4bff-bbbf-2ae9635f4a03
chain2 = main(2)

# ╔═╡ 815425f5-0c2a-4592-8a6c-da7aae0e32b7
C2 = Array(chain2)

# ╔═╡ 109373a4-08e7-42d9-98d3-e490cc9ffe24
chain3 = main(3)

# ╔═╡ 5576735c-862d-41ed-b9b6-0af046f3b69b
C3 = Array(chain3)

# ╔═╡ f08641fb-25ff-4aec-adc4-0b955bb61ac2
matwrite("mousefile.mat", Dict("C1"=>C1, "C2"=>C2, "C3"=>C3))

# ╔═╡ 4067d76b-adc2-43c2-806b-5b98840838d9
plot(chain1)

# ╔═╡ d1217a9e-dbcc-4bc2-9002-6495d6874ca1
plot(chain2)

# ╔═╡ e8255fc6-8b90-4a3d-a7d8-d7a857a31511
plot(chain3)

# ╔═╡ 2f8586e9-00e7-48b0-906e-e40951ed17d3
σ̄ = [mean(C[:,1]) for C in (C1,C2,C3)]

# ╔═╡ 9ccbd15f-1c26-4b7d-8555-eb88b16f85be
D̄ = [mean(C[:,2]) for C in (C1,C2,C3)]

# ╔═╡ 2288f429-723e-4499-8292-bf17dddb7e80
ρ̄ = [mean(C[:,3]) for C in (C1,C2,C3)]
...
manifest_format = "2.0"
project_hash = "8ad0552d04fb3d73f99bd2af57461ad2eb593516"
